...
アドレスピンの接続によってI2Cアドレスが0x44または0x45のどちらかに決まるので、こちらも適切に結線するかオープンにする。
...
`const` 型から 非 `const` 型へは、プリミティブ型であればそのまま値をコピーするので暗黙的にキャストできる。
...
~~何故 `private` ではなく `static` にしたのか、いささか不可解。~~
