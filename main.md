...
ESP32 Arduino環境で開発する場合はハードウェアを意識することはあまり多くないが、頭の片隅においておくと何かと便利である。
...
SCLとSDAは共にプルアップしておく必要があるが、ESP32ではプルアップ抵抗を内蔵しているので外付けする必要はない。
...
割り込み間隔 (秒) は、ディバイダ (1カウントに必要なクロック数 (2～65536)) と周期 (カウント数 (1 ～ 2 ^ 64 - 1)) を設定すると `80 [MHz] / (ディバイダ * 周期)` の式で決定される。
...
CPUはデュアルコアのESP32 (おそらくESP32-D0WD) を搭載している。
